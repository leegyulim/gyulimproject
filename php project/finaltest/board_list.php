...
<body>
 ...
 <table>
     ...
     ?>    
          <tr>
              <td><?=$row["num"]?></td>
              <td style="text-align:left;"><a href="board_view.php?num=<?=$row["num"]?>&page=<?=$page?>"><?=$row["title"]?></a></td>
              <td><?=$row["writer"]?></td>
              <td><?=$row["regtime"]?></td>
              <td><?=$row["hits"]?></td>
          </tr>
 ...
 <?php
	...
	$query = $db->query("select count(*) from board");
	$row = $query->fetch();
	$numRecords = $row[0];
	...
	if ($firstLink > 1) {
		$link = $firstLink - 1;
		echo "<a href=\"board_list.php?page=$link\">이전</a> ";
	}
	...
	if ($lastLink < $numPages) {
		$link = $lastLink + 1;
		echo "<a href=\"board_list.php?page=$link\">다음</a> "; 
    }
?>
 </div>
